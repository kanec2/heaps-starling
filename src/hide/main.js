...
AutoWindow.setupIpc = function() {
	...
	electron_main_IpcMain.on("menu:build",function(event,menuData) {
		...
		console.log("AutoWindow.hx:168:","[AutoWindow] ✅ Menu set from template (" + template.length + " top items)");
	});
};
AutoWindow.processMenuTemplate = function(items,sender) {
	...
	while(_g < items.length) {
		var item = items[_g];
		++_g;
		var processed = { label : item.label, type : item.type != null ? item.type : null, submenu : item.submenu != null ? AutoWindow.processMenuTemplate(item.submenu,sender) : null};
		console.log("AutoWindow.hx:181:","[AutoWindow] item id: " + Std.string(item.id));
		if(item.id != null) {
			var itemId = [item.id];
			processed.click = (function(itemId) {
				return function(menuItem,browserWindow,event) {
					console.log("AutoWindow.hx:185:","[AutoWindow] 🖱 Menu clicked: " + itemId[0]);
					console.log("AutoWindow.hx:186:","[AutoWindow] 📤 Sending to renderer: menu:click");
