...
class AutoWindow {
    ...
    static function setupIpc():Void {
        ...
        IpcMain.on("menu:build", function(event, menuData:Dynamic) {
            ...
            trace("[AutoWindow] ✅ Menu set from template (" + template.length + " top items)");
        });
    }
    ...
    static function processMenuTemplate(items:Array<Dynamic>, sender:Dynamic):Array<Dynamic> {
        ...
        for (item in items) {
            ...
            trace("[AutoWindow] item id: "+item.id);
            if (item.id != null) {
                var itemId = item.id;
                processed.click = function(menuItem:Dynamic, browserWindow:Dynamic, event:Dynamic) {
                    trace("[AutoWindow] 🖱 Menu clicked: " + itemId); // 👈 НОВОЕ
                    trace("[AutoWindow] 📤 Sending to renderer: menu:click");
